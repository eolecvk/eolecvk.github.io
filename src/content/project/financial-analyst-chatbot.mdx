...
The **data ingestion workflow** takes the raw report PDF files and structure them into readable data when addressing user queries.  In a production system, we would have a file management portal where an admin can upload new reports files to extend the knowledge base of the chatbot.
